...
### Hồ sơ người dùng
- **URL:** `GET /users/me` (yêu cầu Bearer token)
- **URL:** `PUT /users/me`
- **Body ví dụ:**
```json
{
...
- **URL:** `GET /products/:id` (public)
- **URL:** `POST /products` (admin, multipart/form-data với trường `image`)
- **URL:** `PUT /products/:id` (admin, hỗ trợ thay ảnh)
- **URL:** `DELETE /products/:id` (admin)

Ảnh sản phẩm được lưu vào thư mục `Server/uploads/` và được serve qua `/uploads/...`.
